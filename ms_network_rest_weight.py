...
psd_to_gmn_conndict = {'connection_type' : 'convergent', 'synapse_model': 'ex', 'weights': 1.}
...
GID_file = open('/home/schrader/Documents/microsaccades/data/'+str(sim_title)+'/network/'+str(sim_nr)+'/GID_info_glob_weight.txt','w+')
...
GID_file.write('gmdet_corr_weight \t'+str(layerIDs[0][0])+'\t'+str(layerIDs[0][len(layerIDs[0])-1])+'\n')
gmdet_sum_spikes,gmdet_times = save_spikes('spikes_gmdet_corr_weight',layerIDs,gmdet_sum_spikes,gmdet_times)
gmdet_sum_spikes200,gmdet_times200 = save_spikes_200('spikes_gmdet_corr_weight',layerIDs,gmdet_sum_spikes200,gmdet_times200)

layerIDs = nest.GetNodes(out_gmn)
GID_file.write('gmn_corr_weight \t'+str(layerIDs[0][0])+'\t'+str(layerIDs[0][len(layerIDs[0])-1])+'\n')
gmn_sum_spikes,gmn_times = save_spikes('spikes_gmn_corr_weight',layerIDs,gmn_sum_spikes,gmn_times)
gmn_sum_spikes200,gmn_times200 = save_spikes_200('spikes_gmn_corr_weight',layerIDs,gmn_sum_spikes200,gmn_times200)
...
for i in range(len(layers)):
    ...
    GID_file.write(str(layer_names[i+1])+'_corr_weight\t'+str(layerIDs[0][0])+'\t'+str(layerIDs[0][len(layerIDs[0])-1])+'\n') #because of parasolic
    modet_sum_spikes[i],modet_times[i] = save_spikes('spikes_'+str(layer_names[i+1])+'_corr_weight',layerIDs,modet_sum_spikes[i],modet_times[i])
    modet_sum_spikes200[i],modet_times200[i] = save_spikes_200('spikes_'+str(layer_names[i+1])+'_corr_weight',layerIDs,modet_sum_spikes200[i],modet_times200[i])
...
ps_file = open('/home/schrader/Documents/microsaccades/data/'+str(sim_title)+'/network/'+str(sim_nr)+'/p_corr_max_spikes_weight.txt','a+')
...
ps_file = open('/home/schrader/Documents/microsaccades/data/'+str(sim_title)+'/network/'+str(sim_nr)+'/p_corr_spike_times_weight.txt','a+')
ps_file.write(str(handle_name)+'\t'+str(exp_nr)+'\t'+str(cond_nr)+'\n'+str(modet_times[0])+'\n'+str(modet_times[1])+'\n'+str(modet_times[2])+'\n'+str(modet_times[3])+'\n'+str(modet_times[4])+'\n'+str(modet_times[5])+'\n')
ps_file.close()

ps200_file = open('/home/schrader/Documents/microsaccades/data/'+str(sim_title)+'/network/'+str(sim_nr)+'/p_corr_max_spikes_weight_200.txt','a+')
...
ps200_file = open('/home/schrader/Documents/microsaccades/data/'+str(sim_title)+'/network/'+str(sim_nr)+'/p_corr_spike_times_weight_200.txt','a+')
