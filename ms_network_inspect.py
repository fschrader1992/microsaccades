...
OGIDs = open('/home/schrader/Documents/microsaccades/data/'+str(sim_title)+'/network/'+str(sim_nr)+'/GID_info_glob_modet_gmn_40_30.txt','r+')
...
out_layers=[]
layer_names = ['parasols','p_0_left_corr_modet_gmn_40_30','p_0_right_corr_modet_gmn_40_30'] # ,'p_0_left_corr','p_0_right_corr'] #,'p_60_up','p_60_down','p_120_up','p_120_down']
...
for ln in layer_names:
    ...
    for line in l_file:
        ...
        if ln == 'p_0_left_corr_modet_gmn_40_30':
            if len(sta) > 0:
        ...
        if ln == 'p_0_right_corr_modet_gmn_40_30':
            if len(sta) > 0:
                fmdpos_x += [tp.GetPosition([LIDs[0][nid-deltaID]])[0][1]]
                fmdpos_y += [tp.GetPosition([LIDs[0][nid-deltaID]])[0][0]]
                for s in sta:
        ...
        if ln == 'parasols':
            if len(sta) > 0:
                parpos_x += [tp.GetPosition([LIDs[0][nid-deltaID]])[0][1]]
                parpos_y += [tp.GetPosition([LIDs[0][nid-deltaID]])[0][0]]
                for s in sta:
                    par_pos_times_list += [(int(s),tp.GetPosition([LIDs[0][nid-deltaID]])[0][1],tp.GetPosition([LIDs[0][nid-deltaID]])[0][0])]
...
ml_times_pos_file = open('/home/schrader/Documents/microsaccades/data/'+str(sim_title)+'/network/'+str(sim_nr)+'/ml_times_pos_corr_modet_gmn_40_30.txt','w+') #_'+str(mdw)+'
for t in ml_pos_times_list:
    ml_times_pos_file.write(str(t)+'\n')
ml_times_pos_file.close()

mr_times_pos_file = open('/home/schrader/Documents/microsaccades/data/'+str(sim_title)+'/network/'+str(sim_nr)+'/mr_times_pos_corr_modet_gmn_40_30.txt','w+') #_'+str(mdw)+'
for t in mr_pos_times_list:
    mr_times_pos_file.write(str(t)+'\n')
mr_times_pos_file.close()

par_times_pos_file = open('/home/schrader/Documents/microsaccades/data/'+str(sim_title)+'/network/'+str(sim_nr)+'/par_times_pos.txt','w+')
for t in par_pos_times_list:
    par_times_pos_file.write(str(t)+'\n')
